...
function drawCld(SY, SM) {
    ...
    var i,sD,s,size;
    cld = new calendar(SY, SM);//当前月的日期对象
    ...
    for (i = 0; i < 42; i++) {
        ...
        sD = i - cld.firstWeek; //“cld.firstWeek”这个月的第一天是星期几
...
function changeWorkDate(SY, SM) {
    var yearValueStr = document.getElementById("yearValue");
    if ( SY != Number(yearValueStr.value)) {
        return;
    }
    cld = new calendar(yearValueStr.value, SM); //当前月日期对象
    //转工作日
    var workDayStr = document.getElementById("hidWeekDay");
    if (workDayStr != null) {
        var workdayTemp = workDayStr.value.split(";");
        for (var i = 0; i < workdayTemp.length; i++) {
            var workMonth = workdayTemp[i].split("-");
            if (workMonth[0] == (SM+1)) {
                var workDay = workMonth[1].split(",");
                for (i = 0; i < workDay.length; i++) {
                    for (var j = 0; j < 42; j++) {
                        var sD = j - cld.firstWeek;
                        if (sD > -1 && sD < cld.length) {
                            if (cld[sD] != null && cld[sD].sDay == workDay[i]) {
                                $("#GD" + j).addClass("changeWeekDay");
                            }    
                        }
...
function changeWeekDate(SY, SM) {
    yearValueStr = document.getElementById("yearValue");
    if ( SY != Number(yearValueStr.value)) {
        return;
    }
    cld = new calendar(yearValueStr.value, SM); //当前月日期对象
    //转工作日
    var workDayStr = document.getElementById("hidWorkDay");
    if (workDayStr != null) {
        var workdayTemp = workDayStr.value.split(";");
        for (var i = 0; i < workdayTemp.length; i++) {
            var workMonth = workdayTemp[i].split("-");
            if (workMonth[0] == (SM + 1)) {
                ...
                for (i = 0; i < workDay.length; i++) {
                    for (var j = 0; j < 42; j++) {
                        ...
                        if (sD > -1 && sD < cld.length)
                        {
                            if (cld[sD] != null && cld[sD].sDay == workDay[i]) {//&& cld[j].week != "日" && cld[j].week != "六"
                                $("#GD" + j).addClass("changeWorkDay");
                            }    
                        }
                    }
                }
...
function clear() {
    for (i = 0; i < 42; i++) {
        ...
        $("#GD" + i).removeClass("changeWeekDay");
        $("#GD" + i).removeClass("changeWorkDay");
    }
...
$(function() {
    ...
    changeWorkDate(tY, tM);//调为工作日
    changeWeekDate(tY, tM);//调为非
...
var dateSelection = {
    ...
    changeView : function() {
        ...
        changeWorkDate(global.currYear, global.currMonth); //调为工作日
        changeWeekDate(global.currYear, global.currMonth); //调为非
    },
    commit : function() {
